...
public class GridWorld {
    ...
    public GridWorld(int width, int height, RandomGenerator random) {
        this.random = random;
        this.width = width;
        this.height = height;
    ...
    public int getRange(int gridContentId) throws Exception {
        if (!(ids.get(gridContentId) instanceof Human))
            throw new Exception("GridContent linked to id doesn't have a range");
    ...
    public Boolean isWithinBounds(GridPosition gridPosition) {
        return gridPosition.getX() >= 0
                && gridPosition.getY() >= 0
                && gridPosition.getX() < width
                && gridPosition.getY() < height;
    }
